  ...
  isForm();                
    ...
    private void ppLembarObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppLembarObat1ActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            param.put("diagnosa",Sequel.cariIsi("select kd_penyakit from diagnosa_pasien"));
            param.put("bb",Sequel.cariIsi("select berat from pemeriksaan_ralan where no_rawat=?",TNoRw.getText()));
            param.put("namapasien",Sequel.cariIsi("select resep_identifikasi_pasien from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("kejelasantulisan",Sequel.cariIsi("select resep_identifikasi_pasien from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("namaobat",Sequel.cariIsi("select resep_tepat_obat from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("benardosis",Sequel.cariIsi("select resep_tepat_dosis from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("waktupemberian",Sequel.cariIsi("select resep_tepat_waktu_pemberian from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("carapemberian",Sequel.cariIsi("select resep_tepat_cara_pemberian from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("polifarmasi",Sequel.cariIsi("select obat_tepat_pasien from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("duplikasi",Sequel.cariIsi("select resep_ada_tidak_duplikasi_obat from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("interaksi",Sequel.cariIsi("select resep_interaksi_obat from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("benarobat2",Sequel.cariIsi("select obat_tepat_obat from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("benarwaktu",Sequel.cariIsi("select obat_tepat_waktu_pemberian from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("benardosis2",Sequel.cariIsi("select obat_tepat_dosis from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("benaridentitas",Sequel.cariIsi("select obat_tepat_pasien from telaah_farmasi where no_resep=?",NoResep.getText()));
            param.put("norm",TNoRm.getText());
            param.put("peresep",NmDokter.getText());
            param.put("noresep",NoResep.getText());
            param.put("petugasfarmasi",Sequel.cariIsi("select nama from petugas where nip=?",akses.getkode()));
            param.put("nik",Sequel.cariIsi("select no_ktp from pasien where no_rkm_medis=?",TNoRm.getText()));
